...
Regression models fit to spatial data can lead to improper inference because observations are not statistically independent. Thus we need to check model residuals for autocorrelation. If the residuals are strongly correlated the model is not specified properly.

In this case we can try to improve the model by adding explanatory variables. If that's not possible (no additional data, or no clue as to what variable to include), we can try a spatial regression model. 

Returning once again to the Columbus crime data, we import the data, fit a linear regression model to statistically explain crime rates using income and housing values. Further, we first need to check whether a spatial model is warranted by testing whether there is significant autocorrelation in the residuals of a regression model.
...
## Fitting and interpreting a spatial lag model
...
model.lag <- spatialreg::lagsarlm(formula = f, 
                                  data = CC.sf, 
                                  listw = wts)
...
The coefficients on income and housing have the same sign (negative) and they remain statistically significant. But we these cannot be interpreted as the marginal effects.
...
Now the weights matrix is post multiplied by the matrix of X variables. This is the spatially lagged X model (Durbin model). Here $W$ is again the weights matrix and $\theta$ is a vector of coefficients for each lagged explanatory variable.

We fit the model using the `lmSLX()` function from the {spatialreg} package.
```{r}
( model.slx <- spatialreg::lmSLX(formula = f, 
                               data = CC.sf, 
                               listw = wts) )
```
...
In this case, the total effect of income on crime is the sum of the direct effect and indirect effect. Again, using the `impacts()` function we see this.
```{r}
spatialreg::impacts(model.slx, listw = wts)
```
...
```{r}
summary(spatialreg::impacts(model.slx, listw = wts))
```

See also: https://youtu.be/b3HtV2Mhmvk
