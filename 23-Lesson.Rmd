...
Last time we were introduced to some of the terminology and maths of spatial statistical interpolation. To review, the steps for spatial interpolation (statistical) are:
...
Today we consider some concrete examples using functions from the {geoR} package.

## Interpolating spatial data using functions from the {geoR} package

The {geoR} package was one of the first comprehensive packages devoted spatial statistical interpolation. The functions are helpful in learning the required steps.
...
The functions in the {geoR} package only work with objects of class `geodata`. Thus we first need to convert the data frame of observations and locations into a `geodata` object.
...
We examine the makeup of the variogram in more detail with a 'variogram cloud'. Consider again the simulated data set `s100`.
```{r}
par(mfrow = c(1, 1))
plot(s100)
```
