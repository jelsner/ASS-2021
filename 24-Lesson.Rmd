...
Last two weeks of what has been a long (and lonely) semester. Here is my plan to bring this course across the finish line. 

Last week: introduction to spatial statistical interpolation. It begins with separating trends from local spatial autocorrelation. The local spatial autocorrelation is quantified with an empirical variogram. The next step involves fitting a model to the empirical variogram.

Today: modeling the variogram and creating an interpolated surface.

## Modeling the variogram

A parametric curve through the set of points that make up the empirical variogram is called the variogram model.

As an example, lets plot the empirical variogram from the `s100` data. The `uvec =` argument controls the range of lag distance values and the number of variogram estimates. 
```{r}
library(geoR)

s100.v <- variog(s100, uvec = seq(0, 1, l = 21))
plot(s100.v)
```

We overlay a curve corresponding to a variogram model using the `lines.variomodel()` function. We specify the type of model with the `cov.model` argument and values for the sill and range parameters (in that order) with the `cov.pars` argument. We specify the nugget with the `nugget` argument.

Here we entertain an exponential model with `cov.model = "exp"` having a sill value of 1 and a range value of .3 and with a nugget of 0. The practical range for the exponential model is the distance along the horizontal axis at which the curve reaches 95% of the sill, which is 3 times the range specified in the `cov.pars` (covariance parameters) argument (here .9). Finally we specify the maximum distance with the `max.dist` argument.
```{r}
plot(s100.v)
...
The red curve corresponds to an exponential model with those parameters. It looks like it fits the variogram estimates reasonably well. It should because the data were generated from this model.

With observed data we don't know the model, so we estimate the parameters using information from the empirical variogram. The estimation procedure can involve
...
Try various curves and choosing the one that looks the best. Here we plot two other models on top of the `s100` empirical variogram and the original model.
```{r}
plot(s100.v)
lines.variomodel(cov.model = "exp", 
                 cov.pars = c(1, .5),
...
<<<<<<< HEAD
The black line is based on the same exponential form, but it has a range of .5 and a nugget of .1. The gray line is based on a spherical model with a sill and range of .8 and a nugget of .1. 
=======
...
>>>>>>> f51359f6ef65eba2c3c3c21286737b379675238f

All three curves fit the points reasonably well. In practice, the choice often makes little difference in the quality of the spatial interpolation.
