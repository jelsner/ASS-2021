...
Kriging is statistical spatial interpolation. It is the centerpiece of what is called 'geostatistics.' The resulting surface (kriged surface) is composed of three parts. (1) Spatial trend: an increase or decrease in the values that depends on direction or a covariate (co-kriging); (2) Local spatial autocorrelation. (3) Random variation. This should now sound familar. Together the three components provide a model that is used to estimate values everywhere within a specified domain.

In short, geostatistics is used to (1) quantify spatial correlation, (2) predict values at specific locations, (3) provide an estimate of uncertainty on the predicted values, and (4) simulation.

As we've done with areal averaged data and point pattern data (Moran I, Ripley K), we begin with understanding how to quantify spatial autocorrelation in geostatistical data.
...
Our interest will focus on a model for the variogram $\gamma(h)$. But to understand the variogram it helps to first consider the covariogram. 
...
The {geoR} package contains functions for spatial interpolation. There are others but it was one of the first comprehensive packages devoted to spatial interpolation. It is useful for learning how things work.
