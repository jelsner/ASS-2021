...
## Brief introduction to working with space-time data using the {stars} package
...
There are two spatial dimensions and the third dimension is time in months. There is no CRS so we assign the geographic EPSG 4326 using the `st_crs()` function.
```{r}
plot(w.stars[,,,1])
```
