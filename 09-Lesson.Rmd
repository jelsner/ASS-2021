...
In the last lesson we learned how to work with raster data with functions from the {raster} package. Functions from that package require data objects to be of S4 reference class, like those vector data objects in the {sp} package. S4 reference classes are used for specialized applications to construct rich and complicated data representations (see the _Bioconductor_ project).

Most packages from CRAN use the S3 reference class. It is simpler (all objects are a list with a class), easier to maintain, and it allows for new dialects (e.g., {dplyr}, {ggplot2}).

Functions from the {stars} package allow us to work with the raster data model as S3 objects.

Consider a multi-band raster taken from a Landsat 7 view of a small part of the Brazilian coast as part of the {stars} package and stored as a _GeoTIFF_ file labeled `L7_ETMs.tif`. We first import it as a raster stack.
```{r}
...
The data `L7.rs` is a `RasterStack` object with as a S4 reference class. 
...
We extract a single band (layer) from the stack with the `layer =` argument in the `raster()` function. We then plot the raster values with the `plot()` method and compute the spatial autocorrelation with the `Moran()` function.
```{r}
...
Space-time data often comes in the form of arrays, with space and time being array dimensions. Examples include
...
But data cubes can be of higher dimensions such as a five-dimensional where along with time and space, the spectral band and the sensor are additional dimensions. [Data cube](https://raw.githubusercontent.com/r-spatial/stars/master/images/cube2.png)
...
Note there are three dimensions, two spatial (`x` and `y`), and the third across six bands (`band`). Values across the six bands and space are summarized as a single attribute with name `L7_ETMs.tif`.

The data `L7.stars` is a `stars` object with a S3 reference class. It has no slots.
```{r}
slotNames(L7.stars)
```

The data are stored in a four dimensional array with the first index the attribute, the second and third indexes the spatial coordinates, and the fourth index the band.
...
The bands can be operated on to generate a new object. For example here we compute the normalized difference vegetation index (NDVI) through a function applied across the `x` and `y` spatial dimensions using the `st_apply()` method after creating the function `nvdi`.
```{r}
...
The package {ggplot2} supports `sf` objects with the function `geom_sf()`. An initial `ggplot()` function followed by one or more layers, that are added with `+`. The layers begin with `geom_`.
...
And `nz_height` is a simple feature data fraome containing the elevation of specific locations in New Zealand.
```{r}
str(nz_height)
```
...
An advantage of using functions from the {ggplot2} package for mapping is that the package has a strong user-community and there are many add-on packages.
...
There are other packages for making nice maps with some of them listed in the syllabus. In particular I like the {tmap} package because it is agnostic to the type of spatial data object. Simple feature data frames, S4 spatial data, and  rasters from the {raster} package can be layers on a single map.
```{r}
...
The `install_github()` function from the {devtools} package is used to install a package on GitHub. GitHub is a company that provides hosting for software development version control using Git. Git is a version-control system for tracking changes in code during software development.
```{r}
if(!require(devtools)) install.packages(pkgs = "devtools", repos = "http://cran.us.r-project.org")
library(devtools)

install_github(repo = "Nowosad/spDataLarge")
library(spDataLarge)
```
...
The start location of the tornado track has a `POINT` geometry. 
