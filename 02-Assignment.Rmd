...
Make sure you have the {tidyverse}, {spData}, and {sf} packages available by running the code chunk below. 
```{r}
...
library(spData)
```
Then answer the five questions below using the appropriate functions `select()`, `filter()`, `summarize()`, and `group_by()`.
...
c. Create a new data frame containing only states from the West region with a 2015 population of more than 6 million people.
