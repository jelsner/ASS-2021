...
Today: Last lesson on point pattern data analysis and modeling. I will show two examples from my research on tornadoes. Friday's assignment (Assignment 9) will be to write a short paragraph (5 or 6 sentences) describing you research interest as it relates (or might relate) to spatial statistics. You will write the paragraph in the Rmd file. I will give you an example. I will post it to Canvas tomorrow (Thu) and it will be due Friday at 3p. I will not be available for Zoom Friday morning.
...
Create a `owin` and `ppp` objects. Note that although we've already subsetted by Kansas tornadoes above we need to subset on the `ppp` object to assign the KS boundary as the analysis window. Keep this in mind: Point pattern analysis and modeling are done with a set of event locations occurring within a boundary. If the boundary is not defined a bounding box is used as the boundary.
```{r}
library(spatstat)
...
We separate the point pattern data into non-violent tornadoes and violent tornadoes. The non-violent tornadoes include those with an EF rating of 0, 1, 2 or 3. The violent tornadoes include those with an EF rating of 4 or 5.
```{r}
NV.ppp <- T.ppp %>%
