...
You should now be set up with R and RStudio. If not, please stay after class today and I will help you.
...
Markdown uses certain symbols for emphasis.

- _italics_
- **bold**, and
- `code font`

Lesson notes are written in text using markdown formatting as needed. Text is interspersed with code. The format for code chunks is
```{r}
# lines of code here
```
...
We then apply functions to the vector data object. A function has a name and parantheses. Inside the parantheses are the function arguments. Many functions have only a single argument like a data vector. Examples include:
```{r}
sum(counts)
length(counts)
...
We get a vector with elements from both `d1` and `d2`.
