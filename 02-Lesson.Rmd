...
This course is a survey of methods used to describe, analyze, and model _spatial_ data using R. Focus is on applications. Emphasis is given to how spatial statistical methods are related through the concept of spatial autocorrelation.

Presentation of the lessons is done remotely. Lab assignments are done without direct supervision. I might have a chance to do some F2F later in the semester. During each lesson I will work through and explain the R code within an `xx-Lesson.Rmd` file on Zoom. The notes in the lesson files are comprehensive, so you can work through them on your own. The notes are written using the markdown language. Think of this as a master class in spatial data science using R.
...
You should now be set up with R and RStudio. If you are not set up yet, please stay after class today and I will help you.
...
## Doing data science with R markdown

If our science is to be convincing, the trail from the data we started with to the final output must be available to the public. It is difficult to create a reproducible trail with a spreadsheet. It is easy to make mistakes (e.g., accidentally sorting just a column rather than the entire table).
...
In data science we go back and forth between:
...
To do these things efficiently we write our code _together_ with comments. This is where R markdown comes in (files that end with `.Rmd`). An R markdown file is a plain text document where text (such as notes or discussion) is interspersed with pieces, or chunks, of R code. When we `Knit` this file the R code is executed piece by piece, in sequence, and either supplementing or replacing the chunks of code with output. 

The resulting file is then converted into a more easily-readable document formatted in HTML, PDF, or Word. The non-code segments of the document are plain text, but they can have simple formatting instructions in them. When we create a _new_ markdown document in R Studio, it contains a sample example.

Markdown treats text surrounded by *asterisks*, **double asterisks**, and `backticks` in special ways.
...
Lesson notes are written in text using markdown formatting as needed. Text is interspersed with code. The format for code chunks looks like this:
```{r}

```

Three back-ticks (on a U.S. keyboard, the character under the escape key) followed by a pair of curly braces containing the name of the language we are using. The back-ticks-and-braces part signal that code is about to begin. We write our code as needed, and then end the chunk with a new line containing three more back-ticks.
...
Finally, note the _Outline_ button in the upper right corner of the markdown file. We can organize and navigate through the file section by section based on the pound symbol (`#`).
...
```{r}
if(!require(LearnBayes)) install.packages(pkgs = "LearnBayes",
                                          repos = "http://cran.us.r-project.org")

library(LearnBayes)
```

Note: When using RStudio we do not need to set up our CRAN repository because it is automatically taken care of. But this is only for interactive use and not when we use {knitr}. And we don't want to install packages that already exist on our computer so we add the conditional `if()` function that says "only install the package if it is not (`!`) available.
...
Working with data frames using functions from the {base} R packages requires that we understand data frames as list objects. This fact makes these functions harder to learn and to remember. In the next lesson we will see a more formalized way to work with data frames.