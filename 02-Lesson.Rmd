...
In this lesson I provide a review (or an introduction) to working with data using functions from the {base} R packages. The {base} R packages are included in your initial installation. They form the scaffolding for working in R, but much of what we will do in this class will involve functions from other packages.

The one exception is that I will introduce functions from the {readr} package (as part of the {tidyverse} set of packages) that simplify importing data into R. These functions are very similar to the corresponding functions in {base} R.

## Working with data using functions from the {base} R packages

Applied spatial statistics is the analysis and modeling of spatial data. To begin our journey we need to know a bit about data objects in R.

Perhaps the most important function is one of the simplest. The `c()` function creates a 'vector' data object (vector object). The function combines (concatenates) individual data values, where each data value is separated by a comma, into a vector object with a length equal to the number of data values.
...
Create a scatter plot of the logaritm of width versus the logarithm of length and with the size of the point proportional to the EF rating.
```{r}
...
I obtained monthly statewide average rainfall (in inches) back to the year 1895 from http://www.esrl.noaa.gov/psd/data/timeseries/. I copied/pasted the values into a text editor and posted the file on the Web at location http://myweb.fsu.edu/jelsner/temp/data/FLprecip.txt. Read the data into R using the `read_table()` function and answer the questions using R code.
```{r}
