...
The {ggplot2} package is the most frequently used visualization tool among data scientists. It is used extensively including the New York Times and 538. It's popular because it is built on principles of good data visualization.
...
Then, to render the scatter plot, we include the function `geom_point()` as a layer with the `+` symbol. Numeric values are specified using the arguments `x =` and `y =` in the `aes()` function and are rendered as points in a two-dimensional cartesian plane.
```{r}
ggplot(data = df, 
...
When we visualize data using {ggplot2} syntax, we are creating a mapping between variables in our data frame and the aesthetic attributes of geometric objects. In the scatter plot above we mapped `foo` to the x-position aesthetic and we mapped `bar` to the y-position aesthetic. This may seem trivial `foo` is the x-axis and `bar` is on the y-axis. We can do that in Excel.
...
Torn.df %>%
  ...
  group_by(EF) %>%
  summarize(Count = n()) %>%
  ggplot(mapping = aes(x = factor(EF), y = Count)) +
    geom_bar(stat = "identity")
```

Here the argument `stat = "identity"` indicates the data are already tabulated. That is, by default `geom_bar()` tables the data if it is integer, character, or factor (like the function `table()`).

Improve the bar chart to make it presentable for publication.
```{r}
Torn.df %>%
  filter(Year >= 2007, EF != -9) %>%
  group_by(EF) %>%
...
The object `us_states` from the {spData} package is a data frame from the U.S. Census Bureau. The variables include the name, region, area, and population.
```{r}
library(spData)
...
This is a simple feature data frame. A data frame that has spatial information stored in the column labeled `geometry`. More about this next lesson.
...
Let's start with a simple scatter plot using logarithmic scales. The variable `AREA` has units so we convert it to a numeric with the `as.numeric()` function. We use the {scales} package
```{r}
library(scales)
