...
Consider the built-in data frame called `studentdata` from the {LearnBayes} package. To get access to this data frame, first install the package with the `install.packages()` function. Put the name of the package {LearnBayes} in quotes (single or double). Then, to make the functions available, use the `library()` function.
```{r}
...
Note: When using RStudio we don't need to set up our CRAN repository. It is done automatically. We can also install packages by clicking on _Packages_ > _Install_ in the lower-right panel. For interactive use and when we use the `Knit` button, we don't want to install packages that already exist on our computer so we add the conditional `if()` function that says "only install the package IF it is not (`!`) available".

Next, make a copy of the data frame by assigning it to the object with name `df`. Finally, print the first six rows using the `head()` function.
```{r}
df <- studentdata
head(df)
```

As noted in the last lesson, data frames are like spreadsheets with rows and columns. The rows are the observations (here each row is a student in an intro stats class at Bowling Green State University) and the columns are the variables. Variables are answers to questions like what is your height?, Choose a number between 1 and 10, Give the time you went to bed last night, etc. 

The names of the columns are available using the `names()` function.
```{r}
names(df)
```
...
Drink preference was one of the questions. Responses are available in the column labeled `Drink` as a vector of character values. So we list all the drink preferences by typing
```{r}
df$Drink
```

Notice that some students left that response blank. These non responses are coded in the data frame as `NA`.
...
There are 113 students who chose milk, 178 chose soda, and 355 chose water. 
...
Students who left that question blank are ignored. To include the missing values we add the argument `useNA = "ifany"` to the `table()` function.
```{r}
table(df$Drink,
      useNA = "ifany")
```

Note: When we want code executed in the text we separate the code using single back ticks. This is useful when we write reports that need updating periodically. We don't need to hard-code values and then search the document for these values to update.
...
Monthly statewide average rainfall (in inches) are available back to the year 1895 from http://www.esrl.noaa.gov/psd/data/timeseries/. I put values into a text editor and then uploaded the file to the Web at location http://myweb.fsu.edu/jelsner/temp/data/FLprecip.txt.

To import the data into R we use the `read_table()` function from the {tidyverse} packages. We assign the object the name `FLp.df`.  We type the name of the object to see that it is a tabled data frame (tibble) with 117 rows and 13 columns.
```{r}
library(tidyverse)
...
In summary, the important {dplyr} verbs are
