...
The distance functions are defined and estimated under the assumption that the point process is stationary (homogeneous). We can treat any sub-region of the domain as an identically and identically distributed sample of the data.
...
Remove duplicate points with the `unique()` function, set the domain to be the county border, and set the name for the unit of length to meters.
```{r}
...
One example of this parent-child process is the Matern cluster model. Parent events come from a homogeneous Poisson process with intensity $\kappa$ and then each parent has a Poisson ($\mu$) number of offspring that are independently and uniformly distributed (iid) within a radius $r$ centered on the parent.
