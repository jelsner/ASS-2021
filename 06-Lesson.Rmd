...
Simple features are data frames with a spatial column. The spatial column is usually called `geometry` (or `geom`). The geometry column is referenced like a regular column.
...
Note that we recycled the name `sfdf`.
...
Length of boundary lines for U.S. states. Transform the CRS. Note that the geometry is multi-polygons. Convert the polygons to multi-linestrings, then use `st_length()` to get the total length of the lines.
```{r}
states <- us_states %>%
