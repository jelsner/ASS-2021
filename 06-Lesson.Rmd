...
Q: What is the difference (syntax wise) between a polygon with a hole and a multi-polygon?
...
As a quick check on the data we just imported, we use `ggplot()` method and the `geom_sf()` layer.
```{r}
ggplot(data = sfdf) +
  geom_sf()
```
...
Before performing geo-computations, we need to set the coordinate reference system including the spatial units. Here we transform the geographic coordinate reference system to a specific Lambert conic conformal (planar) projection with units of kilometers.
```{r}
...
The function `st_as_sfc()` has methods for class `bbox` to generate a polygon around the four bounding box points. Here we apply this function then visualize it.
```{r}
...
The data frame portion of the simple feature data frame contains the same set and values of the attributes.
```{r}
ggplot(data = centers.sfdf) +
  geom_sf()
```

To get the centroid for the state we first create a new simple feature as a union of all the counties.
```{r}
...
ggplot(data = sfdf) +
  geom_sf() +
  geom_sf(data = center.sf, col = "red")
```
...
ggplot(data = sfdf) +
  geom_sf() +
  geom_sf(data = sfdf[mat[, 1],], col = "red") +
  geom_sf(data = center.sf)
```
...
```{r}
buffer.sf <- st_buffer(state.sf, dist = 100)

ggplot(data = buffer.sf) +
  geom_sf() +
