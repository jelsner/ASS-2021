...
Let's see how this works on the Columbus crime data.
```{r}
library(sf)
...
We start by fitting a 'global' regression for the crime rates using income and housing values.
```{r}
f <- CRIME ~ INC + HOVAL
...
## Another example: Southern homicides

The file `south.zip` contains shapefiles with homicide rates and explanatory variables for counties in the southern United States. Download and unzip the file, then import the data using the `read_sf()` from the {sf} package. The data have latitude/longitude coordinates but there is no projection so we set the CRS to long-lat with the `st_crs()` function.
```{r}
...
To see the extent of the data and the spatial geometries make a plot of the county borders.
```{r}
library(ggplot2)
...
We reduce the number of variables in the data frame keeping only the ones of interest using the `select()` function from the {dplyr} package.
```{r}
library(tidyverse)
...
Unfortunately we cannot use neighborhoods defined by contiguity. But to get a sense of what this bandwidth distance means in terms of the average number of neighbors per county we note that one-half the distance squared times pi is the area captured by the bandwidth.
```{r}
( bwA <- pi * (bw * 1000 /2)^2 ) 
```

In units of square meters. Or 21,519 square kilometers.
...
Importantly we can map where resource deprivation has the most influence on the response variable.
```{r}
