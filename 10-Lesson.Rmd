...
Today: Defining spatial neighborhoods and spatial weights. The spatial lag variable and its relationship to autocorrelation.

Finishing up from last lesson, a nice feature of the {tmap} package is that we can create an interactive map using the same code as we used to create a static map. 
...
The graph is a network (topology) showing the contiguity pattern. Tracts close to the center of the city have the most neighbors.
...
Thus when we report a Moran I we need to state what weighting scheme was used.
