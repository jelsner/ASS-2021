...
Today: Map examples using the tornado data. 

## Example: Tornado locations by month
...
Making maps with functions from the {cartography} package: https://www.rdocumentation.org/packages/cartography/versions/2.4.2
...
In the case where data are aggregated to polygons (or raster cells) spatial autocorrelation measures the degree to which attribute values tend to cluster. Recall attributes are variables in a spatial data frame (either as a column in a simple feature object or as a layer in a raster object). 
