...
The nearest neighbor distance function $G(r)$: The cumulative distribution of events having another event within a distance $r$ (event-to-event function).
...
Here we first assign the data to an object called `SP`. We then compute the nearest neighbor distance function using the `Gest()` function with argument `correction = "none"` to specify no correction be made for the borders of the window. We assign the output of this computation to an object called `G`. List the output.
```{r}
SP <- swedishpines
( G <- Gest(SP,
            correction = "none") )
...
Output includes the distance `r`, the raw uncorrected estimate of $G(r)$ and a theoretical estimate based on a homogeneous Poisson process (complete spatial randomness). With the `plot()` method we can easily compare the raw estimates with the  against CSR. Here we add two horizontal lines to help with interpretation.
```{r}
plot(G)
abline(h = c(.2, .5), 
       col = "black",
...
The $J$ function is the ratio of the $F$ function to the $G$ function. For a CSR processes the value of $J$ is one. Here we see a large and systematic departure of $J$ from unity for distances greater than about .5 meter, due to the regularity.
```{r}
J.df <- SP %>%
    Jest() %>%
    as.data.frame() %>%
...
Another commonly used distance function for assessing clustering in point pattern data is called Ripley K function estimated with the function `Kest()` function. Mathematically the function is defined as
$$
...
Here we consider the point pattern for all canes regardless of their age.
