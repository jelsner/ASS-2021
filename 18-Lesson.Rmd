...
Last lesson we saw how the local (spatial varying) intensity of point pattern data can be used to compute the relative risk of events. For example, conditional on the occurrence of a wildfire where is it most likely to be caused by lightning.

Today we examine functions that describe event clustering (and regularity).

## Estimating second-order properties of point pattern data

The local spatial intensity function describes the frequency of events. Intensity is a first-order property (like the average value of a set of numbers). Clustering is a second-order property of spatial data. It answers the question: is the probability of an event in the proximity of another event higher than expected by chance? Think about the location of trees where the tree's seed dispersal leads to a greater likelihood of another tree nearby.

Let $r$ be the distance between two event locations or the distance between an event and an arbitrary point in the domain, then functions to describe clustering include:
...
To assess the degree (and statistical significance) of clustering, we compute a function using our data set and compare the resulting curve (empirical curve) to a theoretical curve assuming a homogeneous Poisson process. The theoretical curve is well defined for homogeneous point patterns (CSR--complete spatial randomness). Deviations of an 'empirical' curve from a theoretical curve provides evidence against CSR. 
...
Here we first assign the data to an object called `SP` (to reduce the amount of typing). We then compute the nearest neighbor distance function using the `Gest()` function with argument `correction = "none"` to specify no correction be made for the borders of the window. We assign the output of this computation to a list object called `G`.
```{r}
SP <- swedishpines
( G <- Gest(SP,
            correction = "none") )
...
The output includes the distance `r`, the raw uncorrected estimate of $G(r)$ (empirical estimate) at various distances, and a theoretical estimate at those same distances based on a homogeneous Poisson process. With the `plot()` method we can easily compare the empirical estimates with the theoretical estimates. Here we add two horizontal lines to help with interpretation.
```{r}
plot(G)
abline(h = c(.2, .5), 
       col = "black",
...
Values of G are on the vertical axis and values of relative distance are on the horizontal axis starting at 0. The black curve is the uncorrected estimate of $G_{raw}(r)$ from the event locations and the red curve is $G_{pois}(r)$ estimated from a homogeneous Poisson process with the same average intensity as the pine saplings. Recall that a homogeneous Poisson model simulates point pattern data that can be described as CSR.

The horizontal dashed line at G = .2 intersects the black line at a distance (r) of 5 units. This means that 20% of the events have another event _within_ 5 units. To check this we compute all pairwise distances with the `pairdist()` function.
```{r}
PD <- pairdist(SP)
PD[1:6, 1:6]

sum(rowSums(PD < 5) - 1) / nrow(PD) * 100
```

This means that 20% of the saplings have another sapling withing .5 meter. The horizontal dashed line at G = .5 intersects the black line at .8 meters indicating that 50% of the pine saplings have another pine sapling within .8 meter.
...
The empty space function (F) is interpreted as the percent of the domain within a distance from any event. Here again we add some lines to help with interpretation. Here we apply the Kaplan-Meier correction for edge effects with `correction = "km"`.
```{r}
F.df <- SP %>%
  Fest(correction = "km") %>%
  as.data.frame() %>%
...
We can see this clearly by using the `envelope()` function with the `fun = Kross`. We first use the `subset()` method with `drop = TRUE` to make a new `ppp` object with only those two groups.
```{r}
