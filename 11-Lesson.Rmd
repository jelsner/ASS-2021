...
## Using other neighborhood definitions

Last time we learned how to compute autocorrelation on areal aggregated data. It involved a weights matrix, which we created using default settings for the neighborhood definition and the weighting scheme.

Other neighborhood definitions are possible.
...
Variables in the simple feature data frame include police expenditures (`POLICE`), crime (`CRIME`), income (`INC`), unemployment (`UNEMP`) and other socio-economic characteristics across Mississippi at the county level. Police expenditures are per capita 1982 (dollars per person). Personal income is per county resident in 1982 (dollars per person). Crime is the number of serious crimes per 100,000 residents in 1981. Unemployment is percent of folks looking for work in 1980.
...
We need to assign the neighborhoods and the associated weights between each region and each neighbor of that region. The default options in the `poly2nb()` and `nb2listw()` functions from the {spdep} package result in a neighborhood definition based on queen contiguity and the weights based on row standardization (the sum of the weights equals the number of regions).
```{r}
nbs <- poly2nb(PE.sf)
wts <- nb2listw(nbs)
```

Alternatively we can specify the number of neighbors and then assign the neighbors based on proximity. We first extract the coordinates of the polygon centroids.
```{r}
coords <- PE.sf %>%
  st_centroid() %>%
  st_coordinates()
...
We then use the `knearneigh()` function and specify that each county for example has six neighbors. Since the CRS is geographic we include the `longlat = TRUE` argument so distances are calculated using great circles.
```{r}
...
The output is a list of five elements with the first being a matrix of dimension number of counties by the number of neighbors. 

Note that the neighborhoods are non-symmetric. County 3 is a neighbor of county 2, but county 2 is not a neighbor of county 3. This is important since some spatial data models require symmetric neighborhood definitions.
...
The argument `sym = TRUE` forces the neighbor list to be symmetric. Let's create another neighborhood object.
```{r}
nbs3 <- knn2nb(knn,
               sym = TRUE)
summary(nbs3)
...
Note that by forcing the same number neighbors for each county creates additional links on counties along the borders. Note also that when the neighbors are defined by proximity counties can be contiguous but not neighbors.
...
I is .218 with a variance of .0045. The $z$ value for the I statistic is 3.41 giving a $p$-value of .0003 under the null hypothesis of no spatial autocorrelation. Thus we reject the null hypothesis and conclude there is weak but significant spatial autocorrelation in unemployment rates across Mississippi at the county level.
...
A check on the distribution of unemployment rates indicates that normality is somewhat suspect. Recall a good way to check the normality assumption is to use the `sm.density()` function from the {sm} package.
```{r}
...
The unemployment rates are a bit more "peaked" (higher kurtosis) than a normal distribution. In this case it is better to use the default `randomisation = TRUE` argument.
...
What do you do with the knowledge that the unemployment rates have significant spatial autocorrelation? By itself not much but it can provide notice that something might be going on in certain regions (hot spot analysis).
...
Get U.S. Census data with functions from the {tidycensus} package. The `get_decennial()` function grants access to the 1990, 2000, and 2010 decennial US Census data and the `get_acs()` function grants access to the 5-year American Community Survey data. For example, here is how you would get county-level population  for Iowa.
```{r}
library(tidycensus)
...
Now get the tornado locations and compute the annual tornado occurrence rate for each county. Start by determining the intersections of the county polygons and the tornado points.
```{r}
...
TorCounts.df <- Torn.sf %>%
  st_intersection(IA_counties.sf) %>%
  st_drop_geometry() %>%
  group_by(GEOID) %>%
  summarize(nT = n()) 
...
IA_counties.sf <- IA_counties.sf %>%
  left_join(TorCounts.df,
            by = "GEOID") %>%
  mutate(Area = st_area(IA_counties.sf),
         rate = nT/Area/(2018 - 1950 + 1) * 10^10,
         lpop = log10(estimate))
```
